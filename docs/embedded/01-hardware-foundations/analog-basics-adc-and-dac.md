...
Now put this board's numbers in. On a Nucleo-F411RE running SYSCLK at 100 MHz, `PCLK2` is also at most 100 MHz (DS10314 Rev 8, Table 14). The ADC prescaler only offers ÷2, ÷4, ÷6 and ÷8 (RM0383 Rev 4, `ADCPRE` in `ADC_CCR`), and Table 65 caps `f_ADC` at `36 MHz` — so ÷2 (50 MHz) is illegal and **÷4, giving `f_ADC = 25 MHz`, is the fastest legal setting** at full core speed. With `C_ADC` at its `7 pF` worst case and `R_ADC` at its `6 kΩ` worst case:
...
- **Absolute accuracy is limited by the regulator's tolerance, not the ADC's.** The `LD39050` is specified at ±2.0% output tolerance at 25 °C (DocID15470 Rev 5). Two percent of 3.3 V is 66 mV — about 82 LSB at 12 bits. Any measurement you express in volts inherits that error unless you calibrate it out or measure the reference itself.
...
`RES[1:0]` in `ADC_CR1` selects the resolution, and the trade is against conversion length (RM0383 Rev 4, §11.12.2):

| `RES[1:0]` | Resolution | Conversion (ADCCLK cycles) | 1 LSB at V<sub>REF+</sub> = 3.3 V |
|---|---|---|---|
| `00` | 12-bit | 15 | 806 µV |
