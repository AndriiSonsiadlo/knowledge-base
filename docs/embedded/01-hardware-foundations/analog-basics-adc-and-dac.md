...
`RES[1:0]` in `ADC_CR1` selects the resolution (RM0383 Rev 4, §11.12.2), and the trade is against conversion length. Note carefully what the cycle counts in the third column are: they are **totals**, with the minimum 3-cycle sampling window already included — RM0383 Rev 4 §11.7 "Fast conversion mode" gives the totals directly: "12 bits: 3 + 12 = 15 ADCCLK cycles / 10 bits: 3 + 10 = 13 / 8 bits: 3 + 8 = 11". Do not add your sampling time to them, or you will count those three cycles twice. For any other `SMP` setting, the total is `t_S + 12` at 12-bit, as the table in the previous section computes.
...
Look at what the current budget actually is. Table 52 gives the negative-injection functional-susceptibility limit as `−5 mA` for a general `FT` pin, and **`−0 mA`** — that is, none at all — for a long list of pins including `PC0`, `PC1`, `PC2`, `PC3`, `PB3`–`PB9`, `PC13`–`PC15`, `PH1`, `NRST`, `BOOT0` and `PDR_ON`. On the Nucleo, `PC1` and `PC0` are exactly the pins the board routes to the Arduino `A4` and `A5` positions respectively when `SB56`/`SB51` are fitted (UM1724 Rev 17, Table 10: "PC1 and PC0 (ADC in) are connected to A4 and A5") — analog inputs, on the header, with a zero-milliamp negative-injection budget.
