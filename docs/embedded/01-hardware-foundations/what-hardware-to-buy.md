...
1. **Not all pins are FT.** Of the 80 I/O rows in the datasheet's pin table, 78 are `FT` and exactly two are marked **TC — "Standard 3.3 V I/O"**: `PA0-WKUP` and `PB5` (STM32F411xC/E datasheet, Table 7 legend and Table 8, "Pin definitions"). Both are on the Arduino headers of this board — `PA0` is `A0` and `PB5` is `D4` (UM1724 Rev 17, Table 16) — so they are exactly the pins a beginner reaches for. A `TC` pin is a plain 3.3 V I/O; nothing in the datasheet says it tolerates 5 V, and driving 5 V into one forward-biases its protection diode into the supply rail, which the current table allows no budget for.
