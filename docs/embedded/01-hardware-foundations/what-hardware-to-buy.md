...
- **All of the I/O is brought out, on two sets of headers.** The Arduino Uno V3 headers let you plug in off-the-shelf shields; the ST morpho headers beside them expose all of the microcontroller's I/O (UM1724 Rev 17, §7.12, "Extension connectors"). You will use the morpho headers, because the interesting pins are never the Arduino ones.
...
UM1724 §6.1, "Getting started", step 3 specifies a **USB Type-A to Mini-B** cable for the Nucleo-64. Mini-B is the older, wider, trapezoidal connector — not Micro-B, and not USB-C. It is the single most common thing missing from a first order, and the board will not power up without it.
:::
...
- **A separate debug probe.** Redundant. The ST-LINK/V2-1 is on the board (UM1724 §7.4, "Embedded ST-LINK/V2-1").
...
1. **Not all pins are FT.** Of the 80 I/O rows in the datasheet's pin table, 78 are `FT` and exactly two are marked **TC — "Standard 3.3 V I/O"**: `PA0-WKUP` and `PB5` (STM32F411xC/E datasheet, Table 7 legend and Table 8, "Pin definitions"). Both are on the Arduino headers of this board — `PA0` is `A0` and `PB5` is `D4` (UM1724 Rev 17, Table 16) — so they are exactly the pins a beginner reaches for. A `TC` pin is a plain 3.3 V I/O by the Table 7 legend — the datasheet's numeric tables are less clear-cut, and [Voltage Levels and Logic](./voltage-levels-and-logic.md) works through that tension — so treat both pins as 3.3 V-only. Driving them above V<sub>DD</sub> forward-biases the protection diode into the supply rail, and the current budget for that is small to nonexistent: Table 12 caps injection on `FT`/`TC` pins at `−5/+0 mA`, and Table 52 ("I/O current injection susceptibility") tightens `PB5` specifically to `−0 mA`.
2. **FT tolerance does not survive every mode.** The datasheet's own note 4 to Table 8 reads: "FT = 5 V tolerant except when in analog mode or oscillator mode (for PC14, PC15, PH0 and PH1)." A 5 V-tolerant pin configured as an ADC input is no longer 5 V tolerant.
...
- STMicroelectronics — [**UM1724**, *STM32 Nucleo-64 boards (MB1136)*](https://www.st.com/resource/en/user_manual/um1724-stm32-nucleo64-boards-mb1136-stmicroelectronics.pdf). The board's own manual: jumper defaults, power options, LED and button pin assignments, connector pinouts, and full schematics. The revision consulted here is Rev 17 (September 2025). Read §6.1 "Getting started" before you plug anything in.
- STMicroelectronics — [**STM32F411xC/E datasheet**](https://www.st.com/resource/en/datasheet/stm32f411re.pdf) (DS10314; the revision consulted here is Rev 8, January 2024). Table 7 legend and Table 8 pin definitions carry the `FT`/`TC` per-pin markings that the 5 V warning above depends on.
