...
| Cortex-M (32-bit MCU) | STM32, NXP LPC/Kinetis, Nordic nRF52/53, TI MSP432 | `arm-none-eabi-gcc` + CMSIS; vendor IDEs (STM32CubeIDE) | Bare-metal or RTOS firmware — the majority of new embedded product designs today |
...
:::warning
"It's an ARM chip" is not enough information to assume code portability. Cortex-M and Cortex-A implement different Armv7/Armv8 architecture *profiles* with different instruction subsets, different (or absent) MMUs, and a completely different exception model — a binary built for a Cortex-A Linux target will not run on a Cortex-M microcontroller, and Thumb-only code assumptions that hold on some Cortex-M parts don't hold across the whole Cortex-M range either. Engineers who assume "ARM" implies binary or even source portability across this landscape lose real time discovering it doesn't at the worst possible point — usually a build or link failure well into a port, not at the design stage where the mismatch would have been cheap to catch.
:::
