...
- STMicroelectronics — [**PM0214**, *STM32 Cortex-M4 MCUs and MPUs programming manual*](https://www.st.com/resource/en/programming_manual/pm0214-stm32-cortexm4-mcus-and-mpus-programming-manual-stmicroelectronics.pdf), Rev 10. §2.3.7 for the exception stack frame; §4.3.10 for `MMFSR` and the `MSTKERR` / `MMARVALID` bits that identify a guard-region hit; §4.5 for the MPU registers.
