...
- **Both `configKERNEL_*` and `configMAX_SYSCALL_*` are raw priority-register bytes**, written directly to `SHPR`/`BASEPRI`. The STM32F411 implements four priority bits in the **upper** nibble of the 8-bit field (PM0214 §4.3.7), so level 5 is `5 << 4` = `0x50` = 80. The `configLIBRARY_*` pair are the human-readable level numbers and are the only ones you should ever edit. Writing `#define configMAX_SYSCALL_INTERRUPT_PRIORITY 5` truncates to `0x00` — and `BASEPRI` of zero means *masking disabled*, so the kernel's critical sections stop masking anything at all.
