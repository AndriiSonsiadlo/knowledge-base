...
- **CDNA versus RDNA are different products.** CDNA targets datacenter compute (no display output, larger wavefronts, matrix cores); RDNA targets consumer graphics and gaming. Code tuned against one AMD architecture is not automatically well-tuned against the other, the same way Ampere-tuned CUDA isn't automatically well-tuned for Hopper. The two also organize their shader cores differently: a CDNA **compute unit (CU)** is the self-contained block with its own SIMD lanes and LDS, while RDNA pairs two CUs into a **work-group processor (WGP)** sharing one LDS and cache between them. Either way, **CU** (or **WGP** on RDNA) is the unit to reason about when thinking about occupancy and resource limits on AMD hardware — not the NVIDIA SM, which maps to neither one directly.
