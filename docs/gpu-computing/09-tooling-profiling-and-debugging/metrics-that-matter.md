...
`l1tex__data_bank_conflicts_pipe_lsu_mem_shared` counts shared-memory accesses that serialized because multiple threads in a warp addressed the same bank. Zero or near-zero is the target; any sustained nonzero count on a kernel that uses shared memory heavily is worth chasing, because a conflict serializes what should be a single-cycle broadcast into multiple cycles. [Shared Memory Tiling](../07-kernel-optimization/shared-memory-tiling.md)'s `TILE + 1` padding is the standard fix for the column-read pattern that causes this most often; the general fix is changing the stride between threads' addresses so it's no longer a multiple of the bank count.

## Warp stall reasons

`smsp__average_warps_issue_stalled_*` is a family of metrics, one per stall reason (`_long_scoreboard` for a memory dependency, `_barrier` for `__syncthreads()`, `_short_scoreboard` for a shared-memory or fixed-latency dependency, and several more), each reporting the average number of warps per cycle stalled for that specific reason. There's no single "good" value — this family exists to break down *why* a low compute-throughput, low-occupancy kernel is idle, not to be read in isolation. Whichever stall reason dominates points directly at the fix: `_long_scoreboard` dominant means go back to the DRAM-throughput and coalescing metrics above; `_barrier` dominant means threads within a block are imbalanced across a `__syncthreads()`; `_short_scoreboard` dominant often means a dependent-instruction chain that more ILP (see the high-ILP case in [Occupancy Tuning](../07-kernel-optimization/occupancy-tuning.md)) or more resident warps could hide.
